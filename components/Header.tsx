...
export default function Header({
  ...
  useEffect(() => {
    if (currentPage === "home") {
      ...
      const searchwob = searchNavbarwob.current;
      const searchIcn = searchIcon.current;

      if (searchBar) {
        setTimeout(() => {
          search.style.opacity = 1;
          searchwob.style.opacity = 1;
        }, 50);
      }
  ...
  const [dropDownMenuUser, setDDownMenuUser] = useState(false);
  ...
  const [secUser, setSecUser] = useState(0);

  var tUser: NodeJS.Timeout;

  const tickUser = () => {
    setSecUser(secUser + 1);
  };
  ...
  // Start of Handle DropwDownMenuUser

  useEffect(() => {
    if (toggleMenuUser === true && dropDownMenuUser === false) {
      setOpenMenuUser(true);
    } else if (toggleMenuUser === false && dropDownMenuUser === false) {
      timerUser();
      if (secUser >= 1) {
        clearTimeout(tUser);
        setOpenMenuUser(false);
      }
    } else if (toggleMenuUser === false && dropDownMenuUser === true) {
      setOpenMenuUser(true);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [openMenuUser, dropDownMenuUser, toggleMenuUser, secUser]);
  ...
  const searchNavbarwob = useRef<"div" | any | {} | never>(null);
        ...
        if (currentPage === "index") {
          return (
            <StyledHeaderIndex>
              ...
              <SignInButton onClick={navigateToLogIn}>
                <span>Sign In</span>
              </SignInButton>
            </StyledHeaderIndex>
          );
            ...
            <>
              <StyledHeaderwob id="home">
                <Nav>
                  ...
                  <NavContentRight>
                    {searchBar ? (
                      <SearchBarContainer ref={searchNavbarwob}>
                        <Search>
                          <svg
                            viewBox="0 0 24 24"
                            fill="none"
                    ...
                    <DropDownMenuUser>
                      <ProfileContainer
                        ...
                        onMouseLeave={() => {
                          setToggleMenuUser(false), timerUser(), setSecUser(0);
                        }}
                      >
                        <img
                      ...
                      {openMenuUser && (
                        <ArrowNDDContainerUser
                          onMouseEnter={() => {
                            setDDownMenuUser(true);
                          }}
                          onMouseLeave={() => {
                            setDDownMenuUser(false), setSecUser(0), timerUser();
                          }}
                        >
                          <div>
                            <ArrowUpLogin />
                          ...
                          <DropDownContainer>
                            <DownMenuLogin>
                              <p onClick={() => dispatch(setLogoutState(true))}>
                                Sign Out
                              </p>
                            </DownMenuLogin>
                          </DropDownContainer>
